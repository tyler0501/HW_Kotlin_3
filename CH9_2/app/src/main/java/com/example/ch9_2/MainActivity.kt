...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_calculate.setOnClickListener {
            when {
                ed_height.length() < 1 -> showToast("Please Enter Your Height")
                ed_weight.length() < 1 -> showToast("Please Enter Your Weight")
                ed_age.length() < 1 -> showToast("Please Enter Your Age")
                else -> runCoroutines()
            }
        }
    }
    ...
    private fun runCoroutines() {
        tv_weight.text = "Standard Weight\n N/A"
        tv_fat.text = "Body Fat\n N/A"
        tv_bmi.text = "BMI\n N/A"
